...
K = int(sys.argv[1])
logNarr = [10,11,12,13,14,16,20,24]
# logNarr = [12]
...
name = "timing"+str(K)+"Ngrow"+str(rangemin)+"-"+str(rangemax)+".csv"
with open(name, 'w') as file:
	...
	for iteri in logNarr:
		...
		# json_data = json.dumps([obj.__dict__ for obj in karray])
		# with open('pkd'+str(N)+'-'+str(K)+'.dat', 'wb') as file2:
		#     pickle.dump(karray, file2)

		# with open('pkd'+str(N)+'-'+str(K)+'.dat', 'rb') as file2:
		#     karray = pickle.load(file2)
