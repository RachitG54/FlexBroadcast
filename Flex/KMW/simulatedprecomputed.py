...
expname = "timing"+str(N)+"Kgrowprecomputedsim"
parname = str(val)
...
# #file input output for timing numbers
# # name = "timing"+str(N)+"_"+str(K)+".csv"
# name = "timing"+str(N)+"Kgrowprecomputedlazy.csv"
with open(name, 'w') as file:
	writer = csv.writer(file,delimiter='\t')
	writer.writerow(["N", "K", "Bsetsize", "CRSgen", "pkdgen", "enc", "dec", "graphtime", "CRSsize", "pkdsize", "skdsize", "ctsize", "precomputetimeenc", "wprecomputeenc", "precomputetimedec", "wprecomputedec", "precomputesizeenc", "precomputesizedec", "precomputegraphtimeenc", "precomputegrouptimeenc", "precomputegraphtimedec", "precomputegrouptimedec", "randstuffenc1", "randstuffenc2"])
	...
	for iteri in range(val,val+1):
		...
		for i in range(cachetest,K):
			# karray[i] = lazykeygenfull(karray[0],crsnew)
		...
		for rep in range(repeat):
			...
			ct1 = Zs * m;
			ct2 = crsnew[0].g2 ** s
			endexp = timeit.default_timer()
			precomputetimeenc[1] += (endexp-startexp)
			for i in range(bucket):
				minind = i*mainK
				maxind = min((i+1)*mainK,K)
				# print(minind,maxind)
				newkarray = karray[minind:maxind]
				ctarr[i] = encfull(crsnew,newkarray,m,True,s,precomputetimeenc)

		end = timeit.default_timer()
		totalprecomputeenctime = (end-start)/repeat

		precomputetimeenc[0] = precomputetimeenc[0]/repeat
		precomputetimeenc[1] = precomputetimeenc[1]/repeat
		precomputetimeenc[3] = precomputetimeenc[3]/repeat
		precomputetimeenc[4] = precomputetimeenc[4]/repeat
		precomputetimeenc[5] = precomputetimeenc[5]/repeat

		wprecomputeenctime = precomputetimeenc[1]
		pretimeenc = totalprecomputeenctime - wprecomputeenctime


		# wprecomputeenctime = wprecomputeenctime - precomputetimeenc[0]
		...
		for rep in range(repeat):
			gtime=[0]
			newm = decfull(crsnew,newkarray,karray[1],ctarr[rightind],True,ct1,ct2,gtime,1,precomputetimedec)
		end = timeit.default_timer()
		# print('Decryption time:', end - start)

		# wprecomputedectime = (end-start)/repeat
		totalprecomputedectime = (end-start)/repeat

		precomputetimedec[0] = precomputetimedec[0]/repeat
		precomputetimedec[1] = precomputetimedec[1]/repeat
		precomputetimedec[3] = precomputetimedec[3]/repeat

		wprecomputedectime = precomputetimedec[1]
		pretimedec = totalprecomputedectime - wprecomputedectime
		# wprecomputedectime = wprecomputedectime - precomputetimedec[0]
		...
		writer.writerow([str(N), K, mainK, crstime, floattrunc(pkdtime), floattrunc(enctime), floattrunc(dectime), floattrunc(precomputetimedec[3]), crssize, pkdsize, skdsize, ctsize, floattrunc(pretimeenc), floattrunc(wprecomputeenctime), floattrunc(pretimedec), floattrunc(wprecomputedectime), precomputetimeenc[2], precomputetimedec[2], 0, 0, 0, 0, floattrunc(precomputetimeenc[0]), floattrunc(0)] )
